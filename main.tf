locals {
  ...
  iface_naming_strategies = {
    "default" = { template = "ens$${index}", start_index = 3 }
    "q35"     = { template = "enp$${index}s0", start_index = 2 }
  }

  iface_naming = (
    var.interface_naming != null
    ? var.interface_naming
    : try(local.iface_naming_strategies[var.machine], local.iface_naming_strategies["default"])
  )

  should_enable_xml = anytrue([
    length(local.gpu_devices) > 0,
    length(local.network_perf_tuning_effective) > 0,
    length(var.shared_filesystems) > 0,
    var.machine == "q35"
  ])
}
